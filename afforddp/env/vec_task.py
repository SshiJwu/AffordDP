...
class VecTaskPC():
    def __init__(self, task, rl_device, clip_observations=5.0, clip_actions=1.0):
        ...
        self.num_states = task.num_states
        self.num_actions = task.num_actions
        self.num_points = task.num_points

        self.obs_space = spaces.Box(np.ones(self.num_obs) * -np.Inf, np.ones(self.num_obs) * np.Inf)
        self.state_space = spaces.Box(np.ones(self.num_states) * -np.Inf, np.ones(self.num_states) * np.Inf)
        self.act_space = spaces.Box(np.ones(self.num_actions) * -1., np.ones(self.num_actions) * 1.)
        self.pointcloud_space = spaces.Box(np.ones(self.num_points) * -np.Inf, np.ones(self.num_points) * np.Inf)
...
class VecTaskPython(VecTask):
    ...
    def step(self, actions):
        ...
        _, success = self.task.step(actions_tensor)

        return self.task.obs_buf.to(self.rl_device), success
...
class VecTaskPythonArmPCPure(VecTask) :

    def __init__(self, task, rl_device, clip_observations=5.0, clip_actions=1.0):
        super().__init__(task, rl_device, clip_observations, clip_actions)
        self.obs_space = spaces.Box(np.ones((self.task.pointCloudDownsampleNum, self.num_obs)) * -np.Inf, np.ones((self.task.pointCloudDownsampleNum, self.num_obs)) * np.Inf)
    ...
    def step(self, actions):
        ...
        obs, rew, done, pose = self.task.step(actions_tensor)

        return torch.clamp(obs, -self.clip_obs, self.clip_obs).to(self.rl_device), rew.to(self.rl_device), done.to(self.rl_device), self.task.extras, pose
