...
def draw_bbox(img, bbox, color='red', width=2, pred=None):
    ...
    if pred is not None:
        draw.text(top_left, str(pred))
    return img

def draw_multi_bbox(img, bboxs, pred_phrases):
    for bbox, pred in zip(bboxs, pred_phrases):
        img = draw_bbox(img, bbox, pred=pred)
    return img
